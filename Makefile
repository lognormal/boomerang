...
ifeq ($(strip $(TENANT)),)
	...
	set -e && curl -vsS --fail $(INSECURE) -H "X-Auth-Token: $$token" $(SOASTA_REST_PREFIX)/domain/$(DOMAIN_ID) 2>/dev/null | $(LOGIT) | php generate-domain-references.php php://stdin $(DEFAULT_VERSION) | $(LOGIT) | curl -vsS $(INSECURE) --data-binary @- -H "X-Auth-Token: $$token" $(SOASTA_REST_PREFIX)/domain/$(DOMAIN_ID) 2>&1 | $(LOGIT)
...
ifeq ($(strip $(TENANT)),)
	...
	set -e && curl -vsS $(INSECURE) --fail -H "X-Auth-Token: $$token" $(SOASTA_REST_PREFIX)/domain/$(DOMAIN_ID) 2>/dev/null | $(LOGIT) | php generate-domain-references.php php://stdin remove | $(LOGIT) | curl -vsS $(INSECURE) --data-binary @- -H "X-Auth-Token: $$token" $(SOASTA_REST_PREFIX)/domain/$(DOMAIN_ID) 2>&1 | $(LOGIT)
