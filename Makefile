...
STANDALONE_PLUGINS :=
LOGNORMAL_PLUGINS := plugins/config-override.js plugins/page-params.js plugins/auto_xhr.js plugins/spa.js plugins/angular.js plugins/backbone.js plugins/ember.js plugins/rt.js plugins/bw.js plugins/navtiming.js plugins/restiming.js plugins/mobile.js plugins/memory.js plugins/cache_reload.js plugins/md5.js plugins/logn_config.js
...
mpulse-test: lognormal-debug
	...
	cat build/boomerang-$(VERSION).$(DATE)-debug.js | sed -e "s,%beacon_dest_host%%beacon_dest_path%,$(SERVER)/,; s,%config_host%%config_path%,$(SERVER)/boomerang/config.js,; s,%client_apikey%,$(API_KEY),; s,%config_url_suffix%,,; s,/\*BEGIN DEBUG TOKEN\*/log:null\, /\*END DEBUG TOKEN\*/,secondary_beacons:[$(SECONDARY_BEACONS)]\,instrument_xhr:true\,,;" > $(API_KEY).js
