# mPulse Boomerang Release Notes
...
* Issue 878: Adding the new Akamai plugin for Boomerang to handle Mapping teams
* Issue 883: Cookie Compression

## 1.612.0
### Bug Fixes:

* Issue 872: PageParams: Don't send page JavaScript errors as Boomerang errors
...
* Issue 862: Script Management/Boomerang Integration
...
* Issue 849: PageParams: Fix for IE8, do not use `Array.indexOf`
...
* Issue 833: PageParams: Ignore `null` values
...
* Issue 743: PCI compliance: PageParams: Allow disabling via `BOOMR_config`
* Issue 744: PageParams: Protect form elements in PCI mode
* Issue 748: PaintTiming plugin; NavTiming: Deprecate `chrome.loadTimes()`

### Bug Fixes:

* Issue 745: PageParams: Limit handler side effects in `excludeXhrFilter`
* Issue 746: PageParams: Remove prioritized page params
...
* Issue 643: UserTiming: Enable through config-override
...
* Issue 629: PageParams: ResourceTiming: Avoid wildcard path if no wildcards
...
* Issue 589: `config.json`: Don't strip `h.key` after `config.json` refresh comes in
...
* Issue 534: PageParams: Retry JavaScript variables
...
* Issue 510: Enable overriding `secondary_beacons` via `BOOMR_config`
...
* Issue 488: PageParams: Add support for string property access in JS variables
...
* Issue 414: SPA: Beacon fix when `config.js` loads late
...
* Issue 346: Fixes a timing issue with Single Page Apps if `config.js` comes in late
...
* Issue 214: Keep `config.js` refresh nodes around for one cycle to ensure the browser doesn't remove them before they're executed
...
* Allows configuration to be overwritten by the user via `BOOMR_config`
...
* Bug 94254: Memory leak - leaving script nodes around after requesting `config.js`
