...
describe("e2e/04-page-params/32-send-all", function() {
	...
	it("Should have set the Custom Metric 1 on the second beacon", function(){
		var b = tf.lastBeacon();
		assert.equal(b["cmet.CM1"], "10");
	});

	it("Should have set the Custom Metric 2 on the second beacon", function(){
		var b = tf.lastBeacon();
		assert.equal(b["cmet.CM2"], "100");
	});
	...
	it("Should have set the additional variable var1 on the second beacon", function(){
		var b = tf.lastBeacon();
		assert.equal(b.var1, "1");
	});

	it("Should have set the additional variable var2 on the second beacon", function(){
		var b = tf.lastBeacon();
		assert.equal(b.var2, "2");
	});
	...
	it("Should have set rt.tstart = 9999999 on the second beacon", function(){
		var b = tf.lastBeacon();
		assert.isDefined(b["rt.tstart"]);
		assert.equal(b["rt.tstart"], "9999999");
	});

	it("Should have set rt.end = 9999999 on the second beacon", function(){
		var b = tf.lastBeacon();
		assert.isDefined(b["rt.end"]);
		assert.equal(b["rt.end"], "9999999");
	});
});
