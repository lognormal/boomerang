...
describe("e2e/06-bugs/101515-00", function() {
	...
	it("Should have the first beacon be a xhr beacon", function() {
		assert.equal(tf.beacons[0]["http.initiator"], "xhr");
	});

	it("Should have the second beacon be a Page Load beacon", function() {
		assert.isUndefined(tf.beacons[1]["http.initiator"]);
	});

	it("Should have the second beacon contain vis.pre", function() {
		assert.equal(tf.beacons[1]["vis.pre"], 1);
	});
